  ...
  return (
    <PageShell>
      <section className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-10 py-8 sm:py-12">
        <h1 className="text-3xl sm:text-4xl font-semibold tracking-tight">{t("cart.title")}</h1>
        <p className="mt-2 text-sm sm:text-base text-muted-foreground">{totalItems} {totalItems === 1 ? t("cart.item") : t("cart.items")}</p>

        {items.length === 0 ? (
          <div className="mt-16 text-center">
            <ShoppingBag className="size-12 mx-auto text-muted-foreground" />
            <p className="mt-4 text-muted-foreground">{t("cart.empty")}</p>
            <Link to="/" className="mt-6 inline-block btn-primary px-6 py-3 rounded-full font-medium">{t("cart.continue")}</Link>
          </div>
        ) : (
            ...
            <div className="lg:col-span-2 space-y-4">
              {items.map((item) => (
                <div key={item.variantId} className="flex gap-4 rounded-3xl border border-border bg-card p-4 sm:p-6 animate-fade-in">
                  <ProductImage src={item.image} alt={item.productTitle} className="size-24 sm:size-32 shrink-0 rounded-2xl" imageClassName="p-2" />
            ...
            <aside className="rounded-3xl border border-border bg-card p-6 lg:p-8 shadow-soft h-fit lg:sticky lg:top-24">
              ...
              <div className="mt-6 space-y-3 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{t("cart.shipping.calc")}</span>
                </div>
              </div>
              ...
              <button onClick={handleCheckout} className="btn-primary mt-6 w-full inline-flex items-center justify-center gap-2 py-3.5 rounded-full font-medium">
                {t("cart.checkout")} <ExternalLink className="size-4" />
              </button>
              <p className="mt-3 text-xs text-muted-foreground text-center">{t("cart.secure")}</p>
            </aside>
          </div>
        )}
      </section>
